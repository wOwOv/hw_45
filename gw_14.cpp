	...
	while (1)
	{
		...
		do
		{
			...
			if (gqcsretval == 0 || cbTransferred == 0)
			{
				...
				{
					AcquireSRWLockExclusive(&ptr->flagLock);
					{
						InterlockedExchange(&ptr->remove, 1);

						InterlockedExchange(&ptr->recvflag, 0);
						if (InterlockedExchange(&ptr->sendflag, 1) == 0)
						{
							InterlockedExchange(&ptr->sendflag, 0);
						}

						if (ptr->remove == 1 && ptr->recvflag == 0 && ptr->sendflag == 0)
						{
							ReleaseSRWLockExclusive(&ptr->flagLock);
			...
			else
			{
				...
				//비동기 입출력 시작
					if (ptr->RecvQ.GetUsedSize() == ptr->RecvQ.DirectEnqueueSize())
					{
						...
						InterlockedExchange(&ptr->recvflag, 1);
